...
// Fallback: try root .env if not found
if (!process.env.PRINTIFY_API_TOKEN) {
  require('dotenv').config({ path: path.join(__dirname, '../../.env') });
}

// Fallback: use hardcoded token if dotenv truncates (dotenv has 2k char limit)
// Expected length is 1301 characters
let PRINTIFY_API_TOKEN = process.env.PRINTIFY_API_TOKEN;
if (!PRINTIFY_API_TOKEN || PRINTIFY_API_TOKEN.length < 1301) {
  console.log('⚠️  Token from .env is truncated or missing, using hardcoded fallback');
  PRINTIFY_API_TOKEN = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..sHLngjJ1A6aAkK27ggbJ3wobBU_5r8TFSW9ndCpWUW4pKEhffuGcnukfSNz2tr0axE98sSkGMFG7_H7gJrW6I5M1djNZnhL6XTpWxddAYvwW9gsCgsIMrdmDD6TP4w9CrG7PrfdtrHy4Ubj3XmlwzkjWEvbwK8ZqOGLaUBK6Hs7nlqZ1isf70Wl6IbMUVDpQmaeaX-Iul-SrzhNkoxVBceLmvLlM3PylViRMq79XvzSDMV5DPerR6Aw9xFYq8rblZk87_b-l8wvLyDUPFT7Z2JDJ8iFgGLLSV00C1ZKCe6_ItVgq2PTRLLtb0EXMxJInU9CV0wA270606e8fdet1vbnGLJj-E9N-NjuwTApyZ7Iw0Tdggl5gc7jQWaPbuWZAZmUaKToEDmTrlU7UHieexyntxYImGcwrJD5j_Vk2FgueWDKvZf_woTLlVbUAVY5Z16_t1sJ1UIzaISuOjrKMG2oz0AZA7URI3KcNzcmrjFXv_7Pg1_y6PXGy20-pQbklaK7RvF0qTaX7JHcGEJhIK3WvSd0CYShJUn3fjCI3Q089ueLybJw2UtGW-vFwLgW82UaJ64iSdAYLjoER8L0LhTEXELZpQOyv9yZKH7kS8tMC8-V0k1NJFFwapU37B8Hr_qDCehAJKPmH4Dky9osTie_e9eQrTdqgvpKqijUoH6o';
}
...
function extractSize(title) {
  // Split by / to handle "Size / Color" or "Color / Size" formats
  const parts = title.split('/').map(p => p.trim());

  // Check for dimension-based sizes (stickers, tumblers, etc.)
  // Match patterns like: 3" × 3", 20oz, 4" x 4", 3″ × 3″
  const dimensionPattern = /^(\d+[""″]\s*[\u00D7×x]\s*\d+[""″]|\d+oz)$/i;

  // Check each part for dimension pattern
...
function extractColor(title) {
  ...
  if (parts.length !== 2) {
    return null; // No color variant (single part) or too many parts
  }

  // Patterns to identify size (not color)
  const dimensionPattern = /^(\d+[""″]\s*[\u00D7×x]\s*\d+[""″]|\d+oz)$/i;
  const clothingSizePattern = /\b(XS|S|M|L|XL|2XL|3XL|4XL|5XL)\b/i;

  // Check each part - if it's a size, the other part is the color
  if (dimensionPattern.test(parts[0]) || clothingSizePattern.test(parts[0])) {
    ...
    // Second part is size, first part is color
    return parts[0];
  }

  // If neither part matches size patterns, assume no color variant
  return null;
}
