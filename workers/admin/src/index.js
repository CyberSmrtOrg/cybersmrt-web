...
<body>
  ...
  <script>
    ...
    async function loadContentManagement(container) {
      container.innerHTML = \`
        <div class="section">
          ...
          <p style="color: #666; margin-bottom: 30px;">Create and manage content across all CyberSmrt platforms from one place.</p>
          ...
          <div style="margin-bottom: 30px;">
            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
              ...
              <button class="content-type-btn" onclick="showContentType('moodle')" data-type="moodle">
                🎓 Moodle Content
              </button>
              <button class="content-type-btn" onclick="showContentType('social')" data-type="social">
                🚀 Social Media
              </button>
            </div>
          </div>
          ...
          <div id="content-type-area">
            ...
            <!-- Moodle Content -->
            <div id="moodle-content" class="content-type-section">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3 style="margin: 0;">Moodle/LMS Content</h3>
                <button class="action-btn" onclick="createNewPost('moodle')">
                  ➕ New Course Content
                </button>
              </div>
              <div class="info" style="margin-bottom: 20px;">
                Manage educational content for the CyberSmrt Learning Management System
              </div>
              <div class="loading">Loading course content...</div>
            </div>

            <!-- Social Media Hub -->
            <div id="social-content" class="content-type-section">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3 style="margin: 0;">Social Media Hub</h3>
                <button class="action-btn" onclick="createSocialPost()">
                  ➕ New Social Post
                </button>
              </div>
              <div class="info" style="margin-bottom: 20px;">
                Post to all connected social media accounts simultaneously
              </div>

              <!-- Connected Accounts -->
              <div style="margin-bottom: 30px;">
                <h4 style="margin-bottom: 15px;">Connected Accounts</h4>
                <div id="social-accounts-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px;">
                  <div class="loading">Loading social connections...</div>
                </div>
              </div>

              <!-- Recent Posts -->
              <div>
                <h4 style="margin-bottom: 15px;">Recent Posts</h4>
                <div class="loading">Loading recent social posts...</div>
              </div>
            </div>
          </div>
        </div>
    ...
    async function loadMoodleContent() {
      const container = document.getElementById('moodle-content');
      const loadingDiv = container.querySelector('.loading');

      loadingDiv.innerHTML = \`
        <div style="text-align: center; padding: 40px; color: #666;">
          <div style="font-size: 48px; margin-bottom: 16px;">🎓</div>
          <p>Moodle content management coming soon</p>
        </div>
      \`;
    }
    ...
    async function loadSocialPosts() {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Failed to load social connections');
            ...
            <div class="social-account-card">
              <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                <svg viewBox="0 0 24 24" width="24" height="24">
                  <path fill="\${platform.color}" d="\${platform.icon}"/>
                </svg>
                <strong>\${platform.name}</strong>
              </div>
              \${isConnected ? \`
                <div style="font-size: 13px; color: #10b981;">✓ Connected</div>
                <div style="font-size: 12px; color: #666;">\${connection.platform_username || connection.platform_user_id}</div>
                <button class="social-connect-btn" style="background: #ef4444;" onclick="disconnectSocial('\${connection.id}', '\${platform.id}')">Disconnect</button>
              \` : \`
                <div style="font-size: 13px; color: #f59e0b;">⚠ Not Connected</div>
                <button class="social-connect-btn" onclick="connectSocial('\${platform.id}')">Connect</button>
              \`}
            </div>
          \`;
        }).join('');
        ...
        grid.innerHTML = '<div class="error">Failed to load social connections</div>';
      }
    }
    ...
    function createSocialPost() {
      alert('Create social media post - Multi-platform posting coming soon!');
    }
    ...
    async function disconnectSocial(connectionId, platform) {
...
`;
    ...
    // Social Media API endpoints - proxy to auth worker
    if (path.startsWith('/api/social/')) {
      const auth = await validateJWT(request.headers.get('Authorization')?.replace('Bearer ', ''), env.JWT_SECRET);
      if (!auth.valid) {
      ...
      const authUrl = new URL('/admin' + path, env.AUTH_API_URL || 'https://auth.cybersmrt.org');
      try {
        const authResponse = await fetch(authUrl.toString(), {
          method: request.method,
          headers: {
